...
def main():
    ...
    fold_json_file = os.path.join(model_dir, f"fold_{fold}_test.json")
    if os.path.exists(fold_json_file):
        ...
        # Create a k-fold cross-validation object and save the json files for all the folds.
        k_folds = KFold(n_splits=5, shuffle=True, random_state=seed)
        for fold_idx, fold_items in enumerate(list(k_folds.split(data))):
            train_idx, val_idx = list(k_folds.split(data))[fold_idx]
            train_files, val_files = [data[i] for i in train_idx], [data[i] for i in val_idx]
            with open(f"{model_dir}/fold_{fold_idx}_test.json", "w") as fp:
                json.dump({
                    "training": train_files,
                    "validation": val_files
                }, fp)

        # Get the files of the current fold.
